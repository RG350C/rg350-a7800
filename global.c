...
# if defined(DINGUX_MODE) || defined(GCW0_MODE)
  ...
  sprintf(dir1, "%s/.atari7800/", getenv("HOME"));
  mkdir(dir1, 0777);
  free(dir1);
  dir2 = malloc(strlen(getenv("HOME")) + 24);
  sprintf(dir2, "%s/.atari7800/save/", getenv("HOME"));
  mkdir(dir2, 0777);
  free(dir2);
  dir3 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir3, "%s/.atari7800/set/", getenv("HOME"));
  mkdir(dir3, 0777);
  free(dir3);
  dir4 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir4, "%s/.atari7800/txt/", getenv("HOME"));
  mkdir(dir4, 0777);
  free(dir4);
  dir5 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir5, "%s/.atari7800/kbd/", getenv("HOME"));
  mkdir(dir5, 0777);
  free(dir5);
  dir6 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir6, "%s/.atari7800/joy/", getenv("HOME"));
  mkdir(dir6, 0777);
  free(dir6);
  dir7 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir7, "%s/.atari7800/cht/", getenv("HOME"));
  mkdir(dir7, 0777);
  free(dir7);
  dir8 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir8, "%s/.atari7800/scr/", getenv("HOME"));
  mkdir(dir8, 0777);
  free(dir8);
  dir9 = malloc(strlen(getenv("HOME")) + 28);
  sprintf(dir9, "%s/.atari7800/roms/", getenv("HOME"));
  mkdir(dir9, 0777);
  free(dir9);
# endif
...
# if defined(DINGUX_MODE) || defined(GCW0_MODE)
  sprintf(ATARI.atari_home_dir, "%s/.atari7800/", getenv("HOME"));
# else
...
loc_atari_load_settings(char *chFileName)
{
  ...
  while (fgets(Buffer, 512, FileDesc) != (char *)0)
  {
    ...
    *Scan = '\0';
    ...
    else if (!strcasecmp(Buffer, "psp_joystick_cursor"))
      ATARI.psp_joystick_cursor = Value;
    else if (!strcasecmp(Buffer, "psp_display_lr"))
      ATARI.psp_display_lr = Value;
    ...
    else if (!strcasecmp(Buffer, "atari_auto_fire_period"))
      ATARI.atari_auto_fire_period = Value;
  }
...
void atari_update_save_name(char *Name)
{
...
int atari_load_state(char *FileName)
{
  char *scan;
  char SaveName[MAX_PATH + 1];
  int format;
...
void atari_treat_command_key(int atari_idx)
{
  int new_render;
  ...
  case ATARIC_FPS:
    ...
    break;
  case ATARIC_JOY:
    ATARI.psp_joystick_cursor = !ATARI.psp_joystick_cursor;
    break;
  case ATARIC_RENDER:
